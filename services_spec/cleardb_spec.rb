...
describe 'Managing ClearDB' do
  ...
  it 'allows users to create, bind, read, write, unbind, and delete the ClearDB service' do
    plan = user.find_service_plan('cleardb-dev', 'spark')
    plan.should be

    service_instance = nil
    monitoring.record_action(:create) do
      service_instance = user.create_service_instance(space, 'cleardb-dev', 'spark')
    end
    ...
    test_app = TestApp.new(app, route.name, service_instance, 'mysql')
