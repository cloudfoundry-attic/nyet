...
describe 'Managing MongoLab' do
  ...
  it 'allows users to create, bind, read, write, unbind, and delete the MongoLab service' do
    plan = user.find_service_plan('mongolab-dev', 'sandbox')
    plan.should be

    service_instance = nil
    monitoring.record_action(:create) do
      service_instance = user.create_service_instance(space, 'mongolab-dev', 'sandbox')
    end
    ...
    test_app = TestApp.new(app, route.name, service_instance, 'mongodb')
