...
describe 'Managing CloudAMQP' do
  ...
  it 'allows users to create, bind, read, write, unbind, and delete the CloudAMQP service' do
    plan = user.find_service_plan('cloudamqp-dev', 'lemur')
    plan.should be

    service_instance = nil
    monitoring.record_action(:create) do
      service_instance = user.create_service_instance(space, 'cloudamqp-dev', 'lemur')
    end
    ...
    test_app = TestApp.new(app, route.name, service_instance, 'amqp')
